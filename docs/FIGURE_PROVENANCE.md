...
| Figure 3 | `fig3_allsubspec_stratified_2020.jpg` | 2026-08-02 | **twostep** `scripts/figure3_allsubspec_stratified_2020.R` | writes `.jpg` directly | `allsubspec_allyears_stratified_LONG.csv` |
...
**Generator location.** Figure 3 (as of 2026-08-02) is generated by a **twostep-native**
script, `scripts/figure3_allsubspec_stratified_2020.R`, which reads a vendored artifact
and so regenerates from this repo alone; it replaced the former GO-only Figure 3
(`fig3_go2020.jpg`, isochrones `stratify_go_2020_access.R`). The remaining six figures
are canonical in isochrones (source of record). Two of those are also vendored into
`twostep/scripts/` as convenience copies: `figure_2sfca_national_11yr.R` (Figure 2) and
...
### Figure 3, `fig3_allsubspec_stratified_2020.jpg` (all seven subspecialties, 2020, by rurality and race)
- **Generator:** twostep `scripts/figure3_allsubspec_stratified_2020.R` (reads the vendored `artifacts/2sfca/figures/allsubspec_allyears_stratified_LONG.csv`, the same series behind Figures 4 to 6; regenerates from twostep alone). **Built:** 2026-08-02.
- **Replaced** the former GO-only Figure 3 (`fig3_go2020.jpg`, isochrones `scripts/stratify_go_2020_access.R`, from `step_4_2sfca_GO_2020.rds` + `ruca_tract_mapping.csv`), which was removed 2026-08-02. The redesigned figure generalizes the same rurality and race/ethnicity disparity view to all seven subspecialties (gynecologic oncology remains a labeled row, so the GO figures cited in the Results still resolve).
